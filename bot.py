#!/usr/bin/env python3
"""
REPORT WISER BOT - FIXED (No Spam)
"""
import asyncio
import sqlite3
import os
...
class ReportWiserBot:
    ...
    async def start(self):
        ...
        # ===== EVENT: ADMIN ADDED (ONLY ONCE) =====
        @self.client.on(events.ChatAction)
        async def on_admin_added(event):
            # Generate unique event ID
            event_id = f"{event.chat_id}_{event.action}"
        ...
        # ===== EVENT: NEW MEMBER (ONLY ONCE) =====
        @self.client.on(events.ChatAction)
        async def on_new_member(event):
            event_id = f"{event.chat_id}_{event.action}_member"
            if event_id in self.processed_events:
                return
            self.processed_events.add(event_id)
