...
# API Endpoints - HIDDEN (Not exposed to users)
API_ENDPOINTS = [
...
def save_code(code, created_by, hours=24):
    ...
    now = datetime.now().isoformat()
    c.execute('''
        INSERT INTO codes (code, created_by, hours, created_at)
        VALUES (?, ?, ?, ?)
...
def redeem_code(code, user_id):
    ...
    c.execute('SELECT code, hours, is_used FROM codes WHERE code = ?', (code,))
    result = c.fetchone()
    ...
    code_val, hours, is_used = result
    
    if is_used:
        conn.close()
        return False, "Code already used"
    
    c.execute('SELECT 1 FROM codes WHERE code = ? AND used_by = ?', (code, user_id))
    if c.fetchone():
        conn.close()
...
class VehicleAPI:
    ...
    def fetch_from_api(self, vehicle_number, endpoint):
        try:
            ...
            logger.info(f"Trying {endpoint['name']} API")  # URL only in logs
...
def main():
    ...
    print(f"✅ API URLs hidden from users")
