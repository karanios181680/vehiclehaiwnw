#!/usr/bin/env python3
"""
REPORT WISER BOT - FIXED (No Spam + Error Free)
"""
import asyncio
import sqlite3
...
class ReportWiserBot:
    ...
    async def start(self):
            ...
            if getattr(event, 'user_added', None):
                ...
                if me.id in uids:
                    try:
                        ...
                        await self.client.send_message(OWNER_ID, f"✅ Added as admin: {title}")
                    except Exception as e:
                        print(f"[-] Admin error: {e}")

        # WELCOME NEW MEMBER / APPROVED REQUEST
        @self.client.on(events.ChatAction)
        async def on_new_member(event):
            ...
            users = []
            # user_joined can be bool or ID/list
        ...
        async def owner_cmd(event):
            ...
            if msg == "/start":
                ...
                txt = "📋 CHANNELS:\n" + "\n".join(f"{i+1}. {title}" for i, (_, title) in enumerate(ch)) if ch else "No channels."
        ...
        await self.client.send_message(OWNER_ID, f"✅ Bot Started! Channels: {self.count()}")
