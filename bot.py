...
def generate_code(hours=24):
    """Generate unique redeem code - FIXED"""
...
def redeem_code(code, user_id):
    """Redeem a code for a user - FIXED"""
    # Make sure code is uppercase
    code = code.upper()
    ...
    if is_used:
        conn.close()
        logger.warning(f"❌ Code already used: {code} - Used by {used_by}")
        return False, "Code already used"
    
    # Check if this user already used this code (shouldn't happen if not used)
    # But just in case
    if used_by == user_id:
        conn.close()
...
def get_all_codes():
    """Get all codes for admin - FIXED"""
    conn = sqlite3.connect('vehicle_bot.db')
    c = conn.cursor()
    c.execute('''
        SELECT code, hours, is_used, used_by, created_at 
        FROM codes 
    ...
    def fetch_from_api(self, vehicle_number, endpoint):
        try:
            ...
            if response.status_code == 200:
                try:
                    data = response.json()
                    return {'success': True, 'data': data, 'api': endpoint['name']}
                except json.JSONDecodeError:
                    return {'success': True, 'data': response.text, 'api': endpoint['name'], 'is_json': False}
            else:
                return {'success': False, 'error': f"HTTP {response.status_code}", 'api': endpoint['name']}
    ...
    def fetch_vehicle_data(self, vehicle_number):
        for endpoint in API_ENDPOINTS:
            ...
            if result.get('success'):
                ...
                logger.warning(f"❌ {endpoint['name']} API failed")
...
def require_credit(func):
    def wrapper(message, *args, **kwargs):
        ...
        bot.reply_to(
            ...
            "📌 Contact owner to purchase credit."
        )
        return None
    return wrapper
...
def main():
    ...
    save_code(test_code, admin_user_id, 24)
    logger.info(f"✅ Test code created: {test_code}")
    
    print("""
    ╔═══════════════════════════════════════════════╗
    ║   VEHICLE BOT v3.2 - REDEEM FIXED            ║
    ║   - Redeem Codes                            ║
    │   - Test Code Created                       ║
    ║   - API URLs Hidden                         ║
    ╚═══════════════════════════════════════════════╝
